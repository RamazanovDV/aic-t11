...
### 7. Профили агентов (Agent Profiles)
Система поддерживает несколько профилей агентов с индивидуальными настройками:

**Встроенные профили:**
| ID | Название | Контекстный файл |
|----|----------|------------------|
| `developer` | Разработчик | ROLE_DEVELOPER.md |
| `analyst` | Аналитик | ROLE_ANALYST.md |
| `devops` | DevOps-инженер | ROLE_DEVOPS.md |
| `qa` | Тестировщик | ROLE_QA.md |
| `lead` | Тимлид | ROLE_LEAD.md |
| `pm` | Продакт менеджер | ROLE_PM.md |
| `security_auditor` | Аудитор ИБ | ROLE_SECURITY_AUDITOR.md |

**Параметры профиля:**
- `provider` — провайдер LLM (пусто = по умолчанию)
- `model` — модель (пусто = по умолчанию)
- `temperature` — температура генерации
- `top_p`, `top_k` — параметры сэмплирования
- `context_file` — путь к контекстному файлу ROLE_*.md

**Использование в чате:**
- Выбор профиля через UI — сохраняется в сессию и применяется к каждому сообщению
- `@теги` в сообщении — одноразонное переключение роли, не сохраняется в сессию
  - `@developer @auditor вопрос` — последовательные ответы от каждого агента

**Контекстные файлы:**
- Системные файлы в `synth/context/ROLE_*.md`
- Пользовательские переопределения в `data/context/ROLE_*.md`
- При сборке сообщений, если у сообщения была роль отличная от текущей, добавляется префикс "Ответ ассистента с ролью X:"

**API:**
- `GET /api/agents` — список всех агентов
- `GET /admin/agents` — список агентов (admin)
- `POST /admin/agents` — создать/обновить агента
- `DELETE /admin/agents/<name>` — удалить агента
- `POST /api/sessions/<id>/agent-role` — установить роль агента для сессии

**Конфигурация:**
- Файл `synth/agents.yaml` — определение агентов
- Вкладка "Агенты" в админке
