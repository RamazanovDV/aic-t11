...
// Response
{
    ...
    "total_tokens": 1500,
    "message_id": "msg_xxx",
    "debug": {...}  // если debug_enabled в сессии
}
```

**Примечание:** Параметр `debug` в payload устарел. Debug режим теперь контролируется через настройки сессии (`debug_enabled`).
...
### POST /api/sessions/<session_id>/context-settings
Сохранить настройки оптимизации контекста и debug.
...
// Request
{
    ...
    "debug_enabled": true  // или false
}
```

### GET /api/sessions/<session_id>/messages/<message_id>/debug
Получить debug информацию сообщения.
